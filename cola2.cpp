#include "cola2.h"
Cola2::Cola2(const QString &Filename,QGraphicsItem *parent): MyItem (Filename, parent)
{
    root=NULL;
    end=NULL;
    pasar=false;
...
void Cola2::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    NodoCarro* temp = root;
    while(temp){
        Autos* c = temp->carro;
...
void Cola2::activarPasar(bool a)
{
    NodoCarro * temp= root;
    while(temp){
        temp->carro->cruzar(a);
...
void Cola2::setName(string name)
{
    this->nombre = name;
...
void Cola2::tomarPosicion(Autos *a, Autos *c)
{
...
void Cola2::desplazarAnt(Autos *c)
{
    float w= c->size().width()+5;
    float h= c->size().height()+5;
    int si = size();
